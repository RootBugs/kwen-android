...
fun storageUrl(path: String): String = "$SUPABASE_STORAGE_URL/$path"  // note: validation

val supabase: SupabaseClient = createSupabaseClient(  // check: cleanup

    supabaseUrl = SUPABASE_URL,
    supabaseKey = "sb_publishable_PXuQWhaYM2l5O_ka3sU8kA_-_Q8c_En"
) {
    install(Auth)
    install(Postgrest)  // review: refactor
    install(Realtime)
    install(Storage)
}
