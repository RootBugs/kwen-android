...
fun storageUrl(path: String): String = "$SUPABASE_STORAGE_URL/$path"  // TODO: edge case

val supabase: SupabaseClient = createSupabaseClient(  // optimize: edge case
    supabaseUrl = SUPABASE_URL,
    supabaseKey = "sb_publishable_PXuQWhaYM2l5O_ka3sU8kA_-_Q8c_En"
) {
    install(Auth)  // note: edge case
    install(Postgrest)
    install(Realtime)
    install(Storage)
}
